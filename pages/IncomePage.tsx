...
const IncomePage: React.FC = () => {
  ...
  const [dateRange, setDateRange] = useState<DateRange>(() => {
    ...
    yesterday.setDate(businessToday.getDate() - 1)
    console.log(
      "[v0] IncomePage init - businessToday:",
  ...
  const setPeriod = (period: PeriodKey) => {
    ...
    console.log("[v0] setPeriod called with:", period, "businessToday:", today.toISOString().split("T")[0])
    ...
    switch (period) {
      ...
      case "ayer":
        ...
        // Verificamos si estamos en la ventana de transicion (entre 1:58 y 2:30 AM)
        const now = new Date()
        const spainTime = new Date(now.toLocaleString("en-US", { timeZone: "Europe/Madrid" }))
        const hours = spainTime.getHours()
        const minutes = spainTime.getMinutes()
        if (hours === 2 && minutes < 30) {
          console.log("[v0] Ayer - En ventana de transicion, datos pueden no estar listos aun")
        }
        break
      case "semana":
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (daysDiff === 1) {
          // Un solo dia (hoy o ayer): comparar con el mismo dia de la semana anterior (7 dias antes)
          prevFrom = new Date(dateRange.from)
          prevFrom.setDate(dateRange.from.getDate() - 7)
          prevTo = new Date(dateRange.to)
          prevTo.setDate(dateRange.to.getDate() - 7)
        } else if (daysDiff <= 7) {
          // Semana parcial o completa: comparar con los mismos dias de la semana anterior
          prevFrom = new Date(dateRange.from)
          prevFrom.setDate(dateRange.from.getDate() - 7)
          prevTo = new Date(dateRange.to)
          prevTo.setDate(dateRange.to.getDate() - 7)
        } else if (daysDiff <= 31) {
          // Mes: comparar con el mismo rango del mes anterior
          prevFrom = new Date(dateRange.from)
          prevFrom.setMonth(dateRange.from.getMonth() - 1)
          prevTo = new Date(dateRange.to)
          prevTo.setMonth(dateRange.to.getMonth() - 1)
          ...
          // Rango largo: comparar con el mismo numero de dias inmediatamente anteriores
          prevTo = new Date(dateRange.from)
          prevTo.setDate(dateRange.from.getDate() - 1)
          prevFrom = new Date(prevTo)
          prevFrom.setDate(prevTo.getDate() - (daysDiff - 1))
        }
  ...
  const getRangeLabel = () => {
    if (dateRange.from.toDateString() === dateRange.to.toDateString()) {
      if (dateRange.from.toDateString() === getBusinessDate().toDateString()) return "Hoy"
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 pb-20">
      <PageHeader
        icon={DollarSign}
        title="Ingresos"
        subtitle={`Análisis financiero y tickets medios: ${getRangeLabel()}`}
        actions={
          <>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-4 text-xs">
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: BRAND_COLORS.success }} />
                  <span className="text-slate-600">Por Comensal</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: BRAND_COLORS.primary }} />
