...
| `fetchCostesTickets(transactionIds)` | `string[]` | `Promise<Record<string, CosteTicket>>` | Vista `vw_coste_ticket` con `.in(...)` → mapa por `transaction_id`. Carga por lote para la columna "Food Cost" del listado. |
