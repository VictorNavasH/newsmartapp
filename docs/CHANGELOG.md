...
### Corregido
- **Auditoría food cost — coste de la botella de vino (A5)**: la botella es una opción con SKU propio (`RST-SDCA-W*-*-B`) que sumaba precio pero **coste 0** → al vender botella solo se contaba el coste de 1 copa. Verificado en GStock (5 copas/botella): coste botella = coste copa × 5 → la opción "Botella" suma coste copa × 4. Aplicado a `product_options.cost_price_option` + puente `option_recipe_map` (manual, revisado). Food cost real por botella ahora 28-42 % (antes ~8 %). Efecto en Bebida 30d: 22,4 % → 23,3 %. Ver `scripts/fix_vino_botella_coste.sql`. *(IVA Maracuya: ya corregido por el usuario en el TPV.)*
