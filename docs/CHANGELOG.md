...
### Cambiado
- **Dashboard — reordenadas las tarjetas** por prioridad operativa: Weather + Reservas → **Live (Facturación Hoy / Ticket Medio / VeriFactu)** → Previsión Facturación Semanal → Progreso vs Objetivos → Top Productos Hoy → Resumen Financiero → Evolución Costes Laborales. Lo operativo y en vivo gana protagonismo arriba; lo financiero-histórico baja. Reorden puro de bloques JSX en `DashboardPage.tsx` (sin cambios de lógica).

### Corregido
- **Auditoría food cost — coste de la botella de vino (A5)**: la botella es una opción con SKU propio (`RST-SDCA-W*-*-B`) cuyo coste era 0 → al vender botella solo se contaba 1 copa. Resuelto **desde GStock** (sin números a mano): cada opción "Botella" mapeada en `option_recipe_map` a su **producto de compra de GStock** (NÜA COLLECTION MENCIA, etc.) con `portion=600` (4 copas extra; la base ya cuenta 1 copa de 150 ml → total = botella de compra completa). El coste lo calcula `fn_refresh_option_costs` (`measurePriceAverage × 600`). Hubo que **ajustar la guarda de plausibilidad** de esa función: aplicaba el coste solo si `≤ products.price` (precio copa), lo que bloqueaba la botella; ahora `≤ products.price + option_price` (precio del ítem con la opción), que sigue bloqueando costes-basura de lote. Food cost real por botella ahora 28-42 %; Bebida 30d 22,4 % → 23,3 %. Ver `scripts/fix_vino_botella_coste.sql` y `scripts/fn_refresh_option_costs.sql`. *(IVA Maracuya: ya corregido por el usuario en el TPV.)*
- **Auditoría food cost — universo de venta (A4)**: `vw_food_cost_real` filtraba `cancelled_at IS NULL` y se dejaba fuera ~10.000 €/mes de **comida ya pagada**. Causa: en este TPV `cancelled_at` es una **marca AUTOMÁTICA de cocina/curso** (se pone a casi todos los platos de comida ~30 ms tras confirmar, casi nunca a bebidas), NO una anulación — esos platos se sirven y se cobran (están en la factura; verificado con la factura 1459/2026 y 565 líneas de comida pagadas con la marca). Corregido a **`is_paid = true`** (lo realmente cobrado, igual que `vw_coste_ticket` y las facturas). Efecto: venta neta 30d 24.517 € → **32.748 €** (completa), food cost Global 22,1 % → **21,8 %** (apenas cambia). La facturación/Ingresos NO se tocan (ya eran correctos); solo se corrige el denominador del KPI de food cost.
...
### Añadido
- **Dashboard — card VeriFactu "Con Error" desplegable.** Al pulsar el contador "Con Error" del card Live de VeriFactu, se despliega **dentro de la misma tarjeta** el detalle de los tickets de HOY (día operativo) que fallaron al crearse en Cuentica, con el descuadre legible (`Cobro X € ≠ total Y €` parseado del error `got/expected`). Nueva vista `vw_verifactu_errores_hoy` (solo lectura, día operativo, `cuentica_status in failed/error`) + `fetchVerifactuErroresHoy()` en `lib/facturacionService.ts`. **Aclaración importante:** ese card cuenta `cuentica_status` (si el envío a Cuentica tuvo éxito), NO el estado real de VeriFactu — un "error" ahí significa que el ticket no se pudo crear en Cuentica (típicamente por importe de cobro descuadrado/pago partido), por eso en Cuentica no aparece como incidencia.
- **Smart Assistant funcional — `/api/chat` ahora habla con un servicio Claude Agent SDK propio en el VPS.** El chat dejó de estar "no configurado". `app/api/chat/route.ts` reenvía la pregunta (con `Bearer` compartido) a `https://smartassistant.nuasmartrestaurant.com/chat`, un servicio Node alojado en el VPS (carpeta `smart-assistant-service/`) que usa el **Claude Agent SDK con Sonnet 4.6 y la suscripción de Claude** (`CLAUDE_CODE_OAUTH_TOKEN` de `claude setup-token`) y consulta Supabase en **solo lectura** vía MCP de Postgres (conexión directa, puerto 5433 — el pooler 6543/Supavisor daba `queue_timeout`). Respuesta **síncrona** `{ output }` (sin webhooks ni Realtime). Nuevas env: `ASSISTANT_API_URL`, `ASSISTANT_API_SECRET`. Sustituye el proxy a `N8N_WEBHOOK_URL` (legado). El agente está restringido a la herramienta de consulta (sin Bash/escritura/internet). Expuesto con HTTPS vía Traefik (`letsencrypt`). Ver `smart-assistant-service/README.md`.
- **Nueva vista Personal (`/personal`)** — sección de plantilla y horas a partir de los datos de Connecteam sincronizados en Supabase. Patrón SPA estándar (item en `NAVIGATION_ITEMS`, `case` en `app/page.tsx`, `React.lazy`). Vista con **5 pestañas** (`MenuBar` como Tesorería) y **selector de mes global** en la cabecera (afecta a las 4 pestañas de periodo; Trabajadores es tiempo real):
  - **Trabajadores** — KPIs de hoy (trabajan / ausentes / plantilla activa), "Hoy en el restaurante" (turno + estado de fichaje cruzando `scheduled_shifts` × `time_activities` × ausencias), próximos 7 días y tabla maestra de plantilla con toggle de archivados. No depende del mes (usa fecha local Madrid).
  - **Resumen** — horas trabajadas/ausencia/computables, horas extra y % cumplimiento por empleado, agregando las semanas ISO del mes (`v_connecteam_horas_extra_semana`).
  - **Puntualidad** — % llegadas tarde, retraso medio y ranking por empleado (`v_connecteam_puntualidad`).
  - **Ausencias** — resumen por tipo y listado del mes (`v_connecteam_ausencias` / `_dia`).
  - **Nocturnas** — horas nocturnas (22:00–06:00) por empleado (`v_connecteam_horas_nocturnas`).
  - Nuevo servicio `lib/personalService.ts` (lectura directa con la `anon key` vía cliente Supabase, sin tocar backend; tablas/vistas Connecteam sin RLS y con SELECT a `anon`), tipos en `types/personal.ts` y sub-componentes en `components/views/personal/`. Pendiente fase 2: **Coste de personal y % sobre ventas** (cruce `connecteam_timesheet_daily.total_pay` × ventas Dotyk).
